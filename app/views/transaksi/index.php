    ...
    <div class="sidebar">
        ...
        <a class="pilih-menu active" href="<?= BASEURL; ?>/transaksi">
    ...
    <div class="container-transaksi">
        <table border="0">
            ...
            <tr>
                <td><?= $no++; ?></td>
                <td>
                    <a style="font-size:15px;color:#03fc90;border-bottom:1px solid #03fc90;" href="<?= BASEURL; ?>/invoice/index/<?= $sale['id_transaksi']; ?>">
                        <?= $sale['id_transaksi']; ?>
                    </a>
                </td>
                <td><?= $sale['jumlah_beli']; ?></td>
                <td><?= $sale['total']; ?></td>
                <td><?= $sale['bayar']; ?></td>
                <td><?= $sale['kembali']; ?></td>
            </tr>
        ...
        <div class="links-nomer">
            <?php if($data['halamanAktif'] > 1) : ?>
                <a href="<?= BASEURL; ?>/transaksi" class="prev">&laquo;&nbsp;</a>
                <a href="<?= BASEURL; ?>/transaksi/index/<?= $data['halamanAktif'] - 1; ?>" class="prev">&lsaquo;&nbsp;</a>
            <?php endif ; ?>
                ...
                <?php if($data['halamanAktif'] >= 3 && $data['halamanAktif'] < $data['jumlahHalaman'] - 2) : ?>
                    <?php for ($i = $data['halamanAktif'] - 2; $i <= $data['halamanAktif'] + 2; $i++) :?> 
                        <?php if( $i == $data['halamanAktif'] ) : ?>
                            <a href="<?=  BASEURL; ?>/transaksi/index/<?= $i; ?>" class="dasar spe"><?= $i; ?></a>
                        <?php else : ?>
                            <a href="<?=  BASEURL; ?>/transaksi/index/<?= $i; ?>" class="dasar"><?= $i; ?></a>
                        <?php endif ; ?>
                    <?php endfor; ?>
            ...
            <?php if($data['halamanAktif'] < $data['jumlahHalaman']) : ?>
                <a href="<?= BASEURL; ?>/transaksi/index/<?= $data['halamanAktif'] + 1; ?>" class="next">&nbsp;&rsaquo;</a>
                <a href="<?= BASEURL; ?>/transaksi/index/<?= $data['jumlahHalaman']; ?>" class="next">&nbsp;&raquo;</a>
            <?php endif ; ?>
        </div>
